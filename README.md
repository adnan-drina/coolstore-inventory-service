...
as a compatibility link or copy, but the file developers should edit is
...
Before using OpenCode for implementation, review [AGENTS.md](AGENTS.md) and the task packet. OpenCode discovers the repo-local `AGENTS.md` rules from the project root, while the provider and model settings live in the local user config at `~/.config/opencode/opencode.json`.
